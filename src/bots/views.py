...
def server_error(request, template_name='500.html'):  # @UnusedVariable
...
def index(request, *kw, **kwargs):  # @UnusedVariable
    ''' when using eg http://localhost:8080
        index can be reached without being logged in.
        most of the time user is redirected to '/home'
    '''
    return shortcuts.render(request, 'admin/base.html')


def home(request, *kw, **kwargs):  # @UnusedVariable
    return shortcuts.render(request, 'bots/about.html', {'botsinfo': botslib.botsinfo()})


def reports(request, *kw, **kwargs):  # @UnusedVariable
    if request.method == 'GET':
        if 'select' in request.GET:                # from menu:select->reports
            form = forms.SelectReports()
            return shortcuts.render(request, form.template, {'form': form})  # go to the SelectReports form
        ...
        if 'fromselect' in request.POST:           # from SelectReports form
            ...
            if not formin.is_valid():
                return shortcuts.render(request, formin.template, {'form': formin})
            ...
            if not formin.is_valid():
                ...
                return shortcuts.render(request, form.template, {'form': form})
    ...
    return shortcuts.render(request, form.template, {'form': form, 'queryset': pquery})


def incoming(request, *kw, **kwargs):  # @UnusedVariable
    if request.method == 'GET':
        if 'select' in request.GET:                 # from menu:select->incoming
            form = forms.SelectIncoming()
            return shortcuts.render(request, form.template, {'form': form})  # go to the SelectIncoming form
        else:                                       # from menu:run->incoming
            cleaned_data = {
                'page': 1,
                'sortedby': 'idta',
                'sortedasc': False,
                'lastrun': bool(viewlib.save_int(request.GET.get('lastrun', 0)))
        ...
        if 'fromselect' in request.POST:            # from SelectIncoming form
            ...
            if not formin.is_valid():
                ...
                if request.user.is_staff or request.user.is_superuser:
                    ...
                    notification = _t('No rights for this operation.')
...
def outgoing(request, *kw, **kwargs):  # @UnusedVariable
    if request.method == 'GET':
...
def document(request, *kw, **kwargs):  # @UnusedVariable
    if request.method == 'GET':
    ...
    query = models.ta.objects.filter(query_utils.Q(status=SPLITUP) | query_utils.Q(status=TRANSLATED))
    pquery = viewlib.filterquery(query, cleaned_data)
...
def process(request, *kw, **kwargs):  # @UnusedVariable
    if request.method == 'GET':
...
def detail(request, *kw, **kwargs):  # @UnusedVariable
    ...
    if request.method == 'GET':
        ...
        return shortcuts.render(request, 'bots/detail.html', {'detaillist': detaillist, 'rootta': rootta})


def confirm(request, *kw, **kwargs):  # @UnusedVariable
    if request.method == 'GET':
        ...
        if 'fromselect' in request.POST:   # from SelectConfirm form
            ...
            if not ta_object.confirmed and ta_object.confirmtype.startswith('ask'):
                ta_object.confirmed = True
                ta_object.confirmidta = '-1'   # to indicate a manual confirmation
                ta_object.save()
                messages.add_message(request, messages.INFO, _t('Manual confirmed.'))
            else:
                messages.add_message(request, messages.INFO, _t('Manual confirm not possible.'))
    ...
    if request.method == 'GET':
        try:
            ...
            if request.GET['action'] == 'downl':
                response = http.HttpResponse(content_type=currentta.contenttype)
            ...
            for ta_object in talijst:
                ...
                if ta_object.filename and ta_object.filename.isdigit():
                    ...
                    ta_object.content = _t('No file available for display.')
            ...
            return shortcuts.render(request, 'bots/filer.html', {'idtas': talijst})
        except Exception:
            return shortcuts.render(request, 'bots/filer.html', {'error_content': _t('No such file.')})
    ...
    if request.method == 'GET':
        try:
            ...
            if botsglobal.ini.get('directories', 'usersys') in src and src.endswith('.py'):  # only python source in usersys!
                ...
                return shortcuts.render(request, 'bots/srcfiler.html', {'src': src, 'html_source': html_source})
            else:
                return shortcuts.render(request, 'bots/srcfiler.html', {'error_content': _t('File %s not allowed.' % src)})
        except Exception:
            return shortcuts.render(request, 'bots/srcfiler.html', {'error_content': _t('No such file.')})


def plugin(request, *kw, **kwargs):
    if request.method == 'GET':
        form = forms.UploadFileForm()
        return shortcuts.render(request, 'bots/plugin.html', {'form': form})
    else:
        if 'submit' in request.POST:  # coming from ViewIncoming, go to outgoing form using same criteria
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    notification = _t('Plugin "%s" is read successful.') % request.FILES['file'].name
                    botsglobal.logger.info(notification)
                    messages.add_message(request, messages.INFO, notification)
                finally:
...
def plugin_index(request, *kw, **kwargs):
    if request.method == 'GET':
        return shortcuts.render(request, 'bots/plugin_index.html')
    else:
        if 'submit' in request.POST:  # coming from ViewIncoming, go to outgoing form using same criteria
            # write backup plugin first
            plugout_backup_core(request, *kw, **kwargs)
...
def plugout_index(request, *kw, **kwargs):  # @UnusedVariable
    if request.method == 'GET':
        ...
        botsglobal.logger.info(_t('Start writing configuration index file "%(file)s".'), {'file': filename})
        try:
...
def plugout_backup_core(request, *kw, **kwargs):  # @UnusedVariable
    ...
    try:
        ...
        notification = _t('Backup plugin "%s" is written successful.') % filename
        botsglobal.logger.info(notification)
        messages.add_message(request, messages.INFO, notification)


def plugout(request, *kw, **kwargs):  # @UnusedVariable
    if request.method == 'GET':
        form = forms.PlugoutForm()
        return shortcuts.render(request, 'bots/plugout.html', {'form': form})
    else:
...
def delete(request, *kw, **kwargs):
    if request.method == 'GET':
        ...
        return shortcuts.render(request, 'bots/delete.html', {'form': form})
    else:
        if 'submit' in request.POST:
            ...
            if form.is_valid():
                ...
                if form.cleaned_data['delconfiguration'] or form.cleaned_data['delcodelists'] or form.cleaned_data['deluserscripts']:
                    # write backup plugin first
                    plugout_backup_core(request, *kw, **kwargs)
                botsglobal.logger.info(_t('Start deleting in configuration.'))
                if form.cleaned_data['deltransactions']:
                    ...
                    notification = _t('Transactions from acceptance-testing deleted.')
                    messages.add_message(request, messages.INFO, notification)
                    botsglobal.logger.info(notification)
                if form.cleaned_data['delconfiguration']:
                    ...
                    notification = _t('Database configuration is deleted.')
                    messages.add_message(request, messages.INFO, notification)
                    botsglobal.logger.info(notification)
                ...
                if form.cleaned_data['deluserscripts']:
                    ...
                    notification = _t('User scripts are deleted (in usersys).')
                    messages.add_message(request, messages.INFO, notification)
                    botsglobal.logger.info(notification)
    ...
    if request.method == 'GET':
        ...
        if botsglobal.ini.getboolean('jobqueue', 'enabled', fallback=False):  # run bots-engine via jobqueue-server; reports back if job is queued
            ...
            try:
                ...
                notification = _t('Errors while trying to run bots-engine: "%s".') % msg
                messages.add_message(request, messages.INFO, notification)
                botsglobal.logger.info(notification)
            else:
...
def sendtestmailmanagers(request, *kw, **kwargs):  # @UnusedVariable
    ...
    try:
        mail_managers(_t('testsubject'), _t('test content of report'))
    except Exception:
        txt = botslib.txtexc()
