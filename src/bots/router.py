...
from .botsconfig import (
    DONE, ERROR, OK, RESEND,
    EXTERNIN, EXTERNOUT,
    FILEIN, FILEOUT,
    MERGED, PROCESS, TRANSLATED
)
...
class crashrecovery(new):
    ...
    def run(self):
        ...
        for row in botslib.query(
            '''SELECT MAX(idta) as crashed_idta FROM ta
    ...
    def get_minta4query_route(self):
        ...
        '''
        for row in botslib.query(
            '''SELECT MIN(idta) as route_idta FROM ta
               WHERE idta > %(rootidta_of_current_run)s
...
class automaticretrycommunication(new):
    ...
    def run(self):
        ...
        if idta_lastretry == 1:
            ...
            return False  # no run
        for row in botslib.query(
            '''SELECT MIN(idta) AS min_idta FROM filereport
        ...
        do_retransmit = False
        for row in botslib.query(
            '''SELECT idta, parent, numberofresends FROM ta
...
class rereceive(new):
    ...
    def run(self):
        ...
        do_retransmit = False
        for row in botslib.query(
            '''SELECT idta FROM filereport WHERE retransmit = %(retransmit)s ''',
            {'retransmit': 1}
        ):
            do_retransmit = True
            botslib.changeq(
