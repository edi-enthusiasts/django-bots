...
from .botsconfig import (
    ...
    BFORMAT, DECIMALS,
    FIELDS, ID, VALUE,
    ISFIELD, SFIELD, SUBFIELDS,
    F_LENGTH, LENGTH, MINLENGTH,
    MIN, MAX, MAXREPEAT,
    MPATH, QUERIES, OK,
    FIXEDLINE, LEVEL, LIN, POS,
    SUBTRANSLATION, TRANSLATED
)
...
def parse_edi_file(**ta_info):
    ...
    try:
        ...
        raise botslib.InMessageError(_t('Unknown editype for incoming message: %(editype)s'), ta_info)
    ...
    try:
        ...
        ediobject.errorlist.append(str(
            botslib.InMessageError(
                _t('[A59]: incoming file has not allowed characters at/after file-position %(pos)s: "%(content)s".'),
    ...
    def initfromfile(self):
        ...
        if leftover:
            raise botslib.InMessageError(
                _t('[A50] line %(line)s pos %(pos)s: Found non-valid data at end of edi file; probably a problem with separators or message structure.'),
    ...
    def _formatfield(self, value, field_definition, structure_record, node_instance):
        ...
        if field_definition[BFORMAT] == 'A':
            if len(value) > field_definition[LENGTH]:
                self.add2errorlist(
                    _t('[F05]%(linpos)s: Record "%(record)s" field "%(field)s" too big (max %(max)s): "%(content)s".\n') %
                    {
            ...
            if len(value) < field_definition[MINLENGTH]:
                self.add2errorlist(
                    _t('[F06]%(linpos)s: Record "%(record)s" field "%(field)s" too small (min %(min)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if field_definition[BFORMAT] == 'D':
                try:
                    ...
                    self.add2errorlist(
                        _t('[F07]%(linpos)s: Record "%(record)s" date field "%(field)s" not a valid date: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                try:
                    ...
                    self.add2errorlist(
                        _t('[F08]%(linpos)s: Record "%(record)s" time field "%(field)s" not a valid time: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
            ...
            if length > field_definition[LENGTH]:
                self.add2errorlist(
                    _t('[F10]%(linpos)s: Record "%(record)s" field "%(field)s" too big (max %(max)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if length < field_definition[MINLENGTH]:
                self.add2errorlist(
                    _t('[F11]%(linpos)s: Record "%(record)s" field "%(field)s" too small (min %(min)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if 'E' in value or 'e' in value:
                self.add2errorlist(
                    _t('[F09]%(linpos)s: Record "%(record)s" field "%(field)s" contains exponent: "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if field_definition[BFORMAT] == 'R':
                ...
                try:  # convert to decimal in order to check validity
                    ...
                    self.add2errorlist(
                        _t('[F16]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                if lendecimal != field_definition[DECIMALS]:
                    self.add2errorlist(
                        _t('[F14]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has invalid nr of decimals: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                try:  # convert to decimal in order to check validity
                    ...
                    self.add2errorlist(
                        _t('[F15]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                if '.' in value:
                    self.add2errorlist(
                        _t('[F12]%(linpos)s: Record "%(record)s" field "%(field)s" has format "I" but contains decimal sign: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                    ...
                    try:  # convert to decimal in order to check validity
                        ...
                        self.add2errorlist(
                            _t('[F13]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                            {
                                'linpos': node_instance.linpos(),
                                'record': self.mpathformat(structure_record[MPATH]),
                                'field': field_definition[ID],
            ...
            if current_lex_record is None or structure_level[structure_index][ID] != current_lex_record[ID][VALUE]:
                if structure_level[structure_index][MIN] and not countnrofoccurences:  # is record is required in structure_level, and countnrofoccurences==0: error;
                    ...
                    try:
                        raise botslib.InMessageError(
                            self.messagetypetxt +
                            _t('[S50]: Line:%(line)s pos:%(pos)s record:"%(record)s": message has an error in its structure; this record is not allowed here. Scanned in message definition until mandatory record: "%(looked)s".'),
                            {
                        ...
                        raise botslib.InMessageError(
                            self.messagetypetxt +
                            _t('[S51]: Missing mandatory record "%(record)s".'),
                    ...
                    if current_lex_record is not None and structure_level == self.defmessage.structure:
                        raise botslib.InMessageError(
                            ...
                            _t('[S50]: Line:%(line)s pos:%(pos)s record:"%(record)s": message has an error in its structure; this record is not allowed here. Scanned in message definition until mandatory record: "%(looked)s".'),
                            {
                                ...
                                'line': current_lex_record[ID][LIN],
                                'pos': current_lex_record[ID][POS],
                                'looked': self.mpathformat(structure_level[structure_index-1][MPATH])
                            }
                        )
            ...
            if SUBTRANSLATION in structure_level[structure_index]:
                ...
                if not messagetype:
                    raise botslib.TranslationNotFoundError(
                        _t('Could not find SUBTRANSLATION "%(sub)s" in (sub)message.'),
                ...
                self.messagecount += 1
                self.messagetypetxt = _t('Message nr %(count)s, type %(type)s, ' % {'count': self.messagecount, 'type': messagetype})
        ...
        if self.defmessage.nextmessage is not None:  # if nextmessage defined in grammar: split up messages
            ...
            for line in self.root.children:
                ...
                if first:
                    ...
                    ta_info = self.ta_info.copy()
                    ta_info.update(oldline.queries)  # update ta_info with information (from previous line) 20100905 @UndefinedVariable
                    ta_info['bots_accessenvelope'] = self.root  # give mappingscript access to envelope
                ...
                newroot.append(line)
                oldline = line  # save line 20100905 @UnusedVariable
            else:
                if not first:
                    ta_info = self.ta_info.copy()
...
class fixed(Inmessage):
    ...
    def _lex(self):
        ...
        try:
            ...
            content = botslib.get_relevant_text_for_UnicodeError(msg)
            raise botslib.InMessageError(
                _t('Characterset problem in file. At/after line %(line)s: "%(content)s"'),
        ...
        if self.ta_info['noBOTSID']:  # if read records contain no BOTSID: add it
            ...
            if recordlength != lenfixed:
                if recordlength > lenfixed and self.ta_info['checkfixedrecordtooshort']:
                    raise botslib.InMessageError(
                        _t('[S52] line %(line)s: Record "%(record)s" too short; is %(pos)s pos, defined is %(defpos)s pos.'),
                ...
                if recordlength < lenfixed and self.ta_info['checkfixedrecordtoolong']:
                    raise botslib.InMessageError(
                        _t('[S53] line %(line)s: Record "%(record)s" too long; is %(pos)s pos, defined is %(defpos)s pos.'),
                        line=lex_record[ID][LIN],
                        record=lex_record[ID][VALUE],
                        pos=lenfixed,
                        defpos=recordlength
...
class var(Inmessage):
    ...
    def _lex(self):
        ...
        if mode_inrecord and self.ta_info.get('allow_lastrecordnotclosedproperly', False):
            ...
            if leftover:
                raise botslib.InMessageError(
                    _t('[A51]: Found non-valid data at end of edi file; probably a problem with separators or message structure: "%(leftover)s".'),
                    {'leftover': leftover}
                )
        ...
        for lex_field in lex_record:
            ...
            if not lex_field[SFIELD]:  # preceded by field-separator
                try:
                    ...
                    self.add2errorlist(
                        _t('[F19] line %(line)s pos %(pos)s: Record "%(record)s" too many fields in record; unknown field "%(content)s".\n') %
                        {
                ...
                try:
                    ...
                    self.add2errorlist(
                        _t('[F17] line %(line)s pos %(pos)s: Record "%(record)s" expect field but "%(content)s" is a subfield.\n') %
                        {
                            'content': lex_field[VALUE],
                            'line': lex_field[LIN],
                            'pos': lex_field[POS],
                    ...
                    self.add2errorlist(
                        _t('[F18] line %(line)s pos %(pos)s: Record "%(record)s" too many subfields in composite; unknown subfield "%(content)s".\n') %
                        {
                            'content': lex_field[VALUE],
                            'line': lex_field[LIN],
                            'pos': lex_field[POS],
                ...
                if field_definition[MAXREPEAT] == 1:
                    if 'ISA' == self.mpathformat(record_definition[MPATH]) and field_definition[ID] == 'ISA11':  # exception for ISA
                        ...
                        self.add2errorlist(
                            _t('[F40] line %(line)s pos %(pos)s: Record "%(record)s" expect not-repeating elemen, but "%(content)s" is repeating.\n') %
                            {
                                'content': lex_field[VALUE],
                                'line': lex_field[LIN],
                                'pos': lex_field[POS],
...
class excel(csv):
    ...
    def initfromfile(self):
        ...
        try:
            ...
            raise ImportError(_t('Dependency failure: editype "excel" requires python library "xlrd".'))
        ...
        try:
            ...
            botsglobal.logger.error(
                _t('Excel extraction failed, may not be an Excel file? Error:\n%(txt)s'),
                {'txt': txt}
            )
            raise botslib.InMessageError(
                _t('Excel extraction failed, may not be an Excel file? Error:\n%(txt)s'),
        ...
        if leftover:
            raise botslib.InMessageError(
                _t('[A52]: Found non-valid data at end of excel file: "%(leftover)s".'),
                {'leftover': leftover}
            )
        del self.lex_records
        self.checkmessage(self.root, self.defmessage)
    ...
    def read_xls(self, infilename):
        ...
        def _formatter(t, v):
            return self.format_excelval(book, t, v, False)

        for row in range(sheet.nrows):
            types = sheet.row_types(row)
            values = sheet.row_values(row)
            xlsdata.append(list(map(_formatter, types, values)))
...
class edifact(var):
    ...
    def _sniff(self):
        ...
        try:
            ...
            raise botslib.InMessageError(_t('[A53]: Edifact file contains only whitespace.'))  # plus some border cases; not possible if mailbag is used.
        ...
        for char in self.rawinput[count:count+20]:
            ...
            if count2 <= 2:
                ...
                if found_tag != 'UNB':
                    raise botslib.InMessageError(_t('[A54]: Found no "UNB" at the start of edifact file.'))  # also: UNA too short. not possible if mailbag is used.
            elif count2 <= 7:
                found_charset += char
            elif count2 == 8:
            ...
            raise botslib.InMessageError(_t('[A55]: Problems with UNB-segment; encountered too many <CR/LF>.'))
        ...
        if has_una_string:
            if found_field_sep != self.ta_info['field_sep'] or found_sfield_sep != self.ta_info['sfield_sep']:
                ...
                raise botslib.InMessageError(_t('[A56]: Separators used in edifact file differ from values indicated in UNA-segment.'))
        else:
            if found_field_sep == '+' and found_sfield_sep == ':':  # assume standard/UNOA separators.
                ...
                raise botslib.InMessageError(_t('[A57]: Edifact file with non-standard separators. An UNA segment should be used.'))
        ...
        try:
            ...
            raise botslib.InMessageError(
                _t('[A58]: Edifact file has unknown characterset "%(charset)s".'),
                {'charset': found_charset}
            )
        # except UnicodeDecodeError as msg:
            # raise botslib.InMessageError(
            #     _t('[A59]: Edifact file has not allowed characters at/after file-position %(content)s.'),
            #     {'content': msg[2]}
            # )
    ...
    def checkenvelope(self):
        ...
        for nodeunb in self.getloop({'BOTSID': 'UNB'}):
            ...
            if unbreference and unzreference and unbreference != unzreference:
                self.add2errorlist(
                    _t('[E01]: UNB-reference is "%(unbreference)s"; should be equal to UNZ-reference "%(unzreference)s".\n') %
            ...
            try:
                if int(unzcount) != messagecount:
                    self.add2errorlist(
                        _t('[E02]: Count of messages in UNZ is %(unzcount)s; should be equal to number of messages %(messagecount)s.\n') %
                        {'unzcount': unzcount, 'messagecount': messagecount}
                    )
            except Exception:
                self.add2errorlist(
                    _t('[E03]: Count of messages in UNZ is invalid: "%(count)s".\n') %
                    {'count': unzcount}
                )
            for nodeunh in nodeunb.getloop({'BOTSID': 'UNB'}, {'BOTSID': 'UNH'}):
                ...
                if unhreference and untreference and unhreference != untreference:
                    self.add2errorlist(
                        _t('[E04]: UNH-reference is "%(unhreference)s"; should be equal to UNT-reference "%(untreference)s".\n') %
                ...
                try:
                    if int(untcount) != segmentcount:
                        self.add2errorlist(
                            _t('[E05]: Segmentcount in UNT is %(untcount)s; should be equal to number of segments %(segmentcount)s.\n') %
                            {'untcount': untcount, 'segmentcount': segmentcount}
                        )
                except Exception:
                    self.add2errorlist(
                        _t('[E06]: Count of segments in UNT is invalid: "%(count)s".\n') %
                        {'count': untcount}
                    )
            for nodeung in nodeunb.getloop({'BOTSID': 'UNB'}, {'BOTSID': 'UNG'}):
                ...
                if ungreference and unereference and ungreference != unereference:
                    self.add2errorlist(
                        _t('[E07]: UNG-reference is "%(ungreference)s"; should be equal to UNE-reference "%(unereference)s".\n') %
                ...
                try:
                    if int(unecount) != groupcount:
                        self.add2errorlist(
                            _t('[E08]: Groupcount in UNE is %(unecount)s; should be equal to number of groups %(groupcount)s.\n') %
                            {'unecount': unecount, 'groupcount': groupcount}
                        )
                except Exception:
                    self.add2errorlist(
                        _t('[E09]: Groupcount in UNE is invalid: "%(count)s".\n') %
                        {'count': unecount}
                    )
                for nodeunh in nodeung.getloop({'BOTSID': 'UNG'}, {'BOTSID': 'UNH'}):
                    ...
                    if unhreference and untreference and unhreference != untreference:
                        self.add2errorlist(
                            _t('[E10]: UNH-reference is "%(unhreference)s"; should be equal to UNT-reference "%(untreference)s".\n') %
                            {'unhreference': unhreference, 'untreference': untreference}
                        )
                    untcount = nodeunh.get({'BOTSID': 'UNH'}, {'BOTSID': 'UNT', '0074': None})
                    segmentcount = nodeunh.getcount()
                    try:
                        if int(untcount) != segmentcount:
                            self.add2errorlist(
                                _t('[E11]: Segmentcount in UNT is %(untcount)s; should be equal to number of segments %(segmentcount)s.\n') %
                                {'untcount': untcount, 'segmentcount': segmentcount}
                            )
                    except Exception:
                        self.add2errorlist(
                            _t('[E12]: Count of segments in UNT is invalid: "%(count)s".\n') %
                            {'count': untcount}
                        )
    ...
    def handleconfirm(self, ta_fromfile, error):
        ...
        for nodeunb in self.getloop({'BOTSID': 'UNB'}):
            ...
            tscript, toeditype, tomessagetype = botslib.lookup_translation(  # @UnusedVariable
                fromeditype=editype,
            ...
            if translationscript and hasattr(translationscript, 'main'):
                ...
                # try to run the user mapping script fuction 'change' (after the default mapping); 'change' function recieves the tree as written by default mapping, function can change tree.
...
class x12(var):
    ...
    def _sniff(self):
        ...
        for char in rawinput:
            ...
            if count == 0:
                raise botslib.InMessageError(_t('[A61]: Edi file contains only whitespace.'))  # not with mailbag
            else:
                raise botslib.InMessageError(_t('[A62]: Expect X12 file but envelope is not right.'))
    ...
    def checkenvelope(self):
        ...
        for nodeisa in self.getloop({'BOTSID': 'ISA'}):
            ...
            if isareference and ieareference and isareference != ieareference:
                self.add2errorlist(
                    _t('[E13]: ISA-reference is "%(isareference)s"; should be equal to IEA-reference "%(ieareference)s".\n') %
            ...
            try:
                if int(ieacount) != groupcount:
                    self.add2errorlist(
                        _t('[E14]: Count in IEA-IEA01 is %(ieacount)s; should be equal to number of groups %(groupcount)s.\n') %
                        {'ieacount': ieacount, 'groupcount': groupcount}
                    )
            except Exception:
                self.add2errorlist(
                    _t('[E15]: Count of messages in IEA is invalid: "%(count)s".\n') %
                    {'count': ieacount}
                )
            for nodegs in nodeisa.getloop({'BOTSID': 'ISA'}, {'BOTSID': 'GS'}):
                ...
                if gsreference and gereference and gsreference != gereference:
                    self.add2errorlist(
                        _t('[E16]: GS-reference is "%(gsreference)s"; should be equal to GE-reference "%(gereference)s".\n') %
                ...
                try:
                    if int(gecount) != messagecount:
                        self.add2errorlist(
                            _t('[E17]: Count in GE-GE01 is %(gecount)s; should be equal to number of transactions: %(messagecount)s.\n') %
                            {'gecount': gecount, 'messagecount': messagecount}
                        )
                except Exception:
                    self.add2errorlist(
                        _t('[E18]: Count of messages in GE is invalid: "%(count)s".\n') %
                        {'count': gecount}
                    )
                for nodest in nodegs.getloop({'BOTSID': 'GS'}, {'BOTSID': 'ST'}):
                    ...
                    if streference and sereference and streference != sereference:
                        self.add2errorlist(
                            _t('[E19]: ST-reference is "%(streference)s"; should be equal to SE-reference "%(sereference)s".\n') %
                    ...
                    try:
                        if int(secount) != segmentcount:
                            self.add2errorlist(
                                _t('[E20]: Count in SE-SE01 is %(secount)s; should be equal to number of segments %(segmentcount)s.\n') %
                                {'secount': secount, 'segmentcount': segmentcount}
                            )
                    except Exception:
                        self.add2errorlist(
                            _t('[E21]: Count of segments in SE is invalid: "%(count)s".\n') %
                            {'count': secount}
                        )
        ...
        for nodegs in self.getloop({'BOTSID': 'ISA'}, {'BOTSID': 'GS'}):
            ...
            if translationscript and hasattr(translationscript, 'main'):
                ...
                for nodest in nodegs.getloop({'BOTSID': 'GS'}, {'BOTSID': 'ST'}):
                    ...
                    lou.put(
                        {'BOTSID': 'AK2'},
                        {'BOTSID': 'AK5', 'AK501': AcknowledgeCode}
                    )
        ...
        for nodestx in self.getloop({'BOTSID': 'STX'}):
            ...
            try:
                if int(endcount) != messagecount:
                    self.add2errorlist(
                        _t('[E22]: Count in END is %(endcount)s; should be equal to number of messages %(messagecount)s.\n') %
                        {'endcount': endcount, 'messagecount': messagecount}
                    )
            except Exception:
                self.add2errorlist(_t('[E23]: Count of messages in END is invalid: "%(count)s".\n') % {'count': endcount})
            firstmessage = True
            for nodemhd in nodestx.getloop({'BOTSID': 'STX'}, {'BOTSID': 'MHD'}):
                ...
                try:
                    if int(mtrcount) != segmentcount:
                        self.add2errorlist(
                            _t('[E24]: Count in MTR is %(mtrcount)s; should be equal to number of segments %(segmentcount)s.\n') %
                            {'mtrcount': mtrcount, 'segmentcount': segmentcount}
                        )
                except Exception:
                    self.add2errorlist(_t('[E25]: Count of segments in MTR is invalid: "%(count)s".\n') % {'count': mtrcount})
        ...
        if self.ta_info['messagetype'] == 'mailbag':
            ...
            for item in mailbagsearch:
                if 'xpath' not in item or 'messagetype' not in item:
                    raise botslib.InMessageError(_t('Invalid search parameters in xml mailbag.'))
                # print('search' , item)
                ...
                raise botslib.InMessageError(_t('Could not find right xml messagetype for mailbag.'))
        ...
        for xmlchildnode in xmlnode:  # for every node in mpathtree
            ...
            if not entitytype:  # is a field, or unknown that looks like a field
                ...
                if self.ta_info['checkunknownentities']:
                    self.add2errorlist(
                        _t('[S02]%(linpos)s: Unknown xml-tag "%(recordunkown)s" (within "%(record)s") in message.\n') %
        ...
        if isinstance(jsonobject, list):
            ...
            for child in self.root.children:
                if not child.record:  # sanity test: the children must have content
                    raise botslib.InMessageError(_t('[J51]: No usable content.'))
            ...
            if not self.root:
                raise botslib.InMessageError(_t('[J52]: No usable content.'))
            self.checkmessage(self.root, self.defmessage)
            self.ta_info.update(self.root.queries)
        else:
            # root in JSON is neither dict or list.
            raise botslib.InMessageError(_t('[J53]: Content must be a "list" or "object".'))
    ...
    def _dojsonlist(self, jsonobject, name):
        ...
        for i in jsonobject:
            if isinstance(i, dict):  # check list item is dict/object
                ...
                raise botslib.InMessageError(_t('[J54]: List content must be a "object".'))
        return lijst
    ...
    def _dojsonobject(self, jsonobject, name):
        ...
        for key, value in jsonobject.items():
            if value is None:
                ...
                if self.ta_info['checkunknownentities']:
                    raise botslib.InMessageError(
                        _t('[J55]: Key "%(key)s" value "%(value)s": is not string, list or dict.'),
                        {'key': key, 'value': value}
                    )
                thisnode.record[key] = str(value)
