...
def set_context(request):
    ...
    if not my_context:
        ...
        my_context['bots_environment_text'] = botsglobal.ini.get('webserver', 'environment_text', fallback=' ')
        my_context['bots_environment_text_color'] = botsglobal.ini.get('webserver', 'environment_text_color', fallback='#000000')
        my_context['botslogo'] = botsglobal.ini.get('webserver', 'botslogo', fallback="bots/botslogo.html")
        my_context['bots_touchscreen'] = botsglobal.ini.getboolean('webserver', 'bots_touchscreen', fallback=False)
        my_context['bots_mindate'] = 0 - botsglobal.ini.getint('settings', 'maxdays', fallback=30)
        my_context['menu_automaticretrycommunication'] = botsglobal.ini.getboolean('webserver', 'menu_automaticretrycommunication', fallback=False)
        # in bots.ini it is possible to add custom menu's
        if botsglobal.ini.has_section('custommenus'):
            my_context['custom_menuname'] = botsglobal.ini.get('custommenus', 'menuname', fallback='Custom')
    ...
    if botsglobal.ini.get('webserver', 'menu_all_routes', fallback='') == 'notindefaultrun':
    ...
    elif botsglobal.ini.getboolean('webserver', 'menu_all_routes', fallback=False):
