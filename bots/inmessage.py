    ...
    def _formatfield(self, value, field_definition, structure_record, node_instance):
        ...
        if field_definition[BFORMAT] == 'A':
            if len(value) > field_definition[LENGTH]:
                self.add2errorlist(
                    _('[F05]%(linpos)s: Record "%(record)s" field "%(field)s" too big (max %(max)s): "%(content)s".\n') %
                    {
            ...
            if len(value) < field_definition[MINLENGTH]:
                self.add2errorlist(
                    _('[F06]%(linpos)s: Record "%(record)s" field "%(field)s" too small (min %(min)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if field_definition[BFORMAT] == 'D':
                try:
                    ...
                    self.add2errorlist(
                        _('[F07]%(linpos)s: Record "%(record)s" date field "%(field)s" not a valid date: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                try:
                    ...
                    self.add2errorlist(
                        _('[F08]%(linpos)s: Record "%(record)s" time field "%(field)s" not a valid time: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
            ...
            if length > field_definition[LENGTH]:
                self.add2errorlist(
                    _('[F10]%(linpos)s: Record "%(record)s" field "%(field)s" too big (max %(max)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if length < field_definition[MINLENGTH]:
                self.add2errorlist(
                    _('[F11]%(linpos)s: Record "%(record)s" field "%(field)s" too small (min %(min)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if 'E' in value or 'e' in value:
                self.add2errorlist(
                    _('[F09]%(linpos)s: Record "%(record)s" field "%(field)s" contains exponent: "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if field_definition[BFORMAT] == 'R':
                ...
                try:  # convert to decimal in order to check validity
                    ...
                    self.add2errorlist(
                        _('[F16]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                if lendecimal != field_definition[DECIMALS]:
                    self.add2errorlist(
                        _('[F14]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has invalid nr of decimals: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                try:  # convert to decimal in order to check validity
                    ...
                    self.add2errorlist(
                        _('[F15]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                if '.' in value:
                    self.add2errorlist(
                        _('[F12]%(linpos)s: Record "%(record)s" field "%(field)s" has format "I" but contains decimal sign: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                    ...
                    try:  # convert to decimal in order to check validity
                        ...
                        self.add2errorlist(
                            _('[F13]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                            {
                                'linpos': node_instance.linpos(),
                                'record': self.mpathformat(structure_record[MPATH]),
                                'field': field_definition[ID],
        ...
        if self.ta_info['noBOTSID']:  # if read records contain no BOTSID: add it
            ...
            if recordlength != lenfixed:
                if recordlength > lenfixed and self.ta_info['checkfixedrecordtooshort']:
                    raise botslib.InMessageError(
                        _('[S52] line %(line)s: Record "%(record)s" too short; is %(pos)s pos, defined is %(defpos)s pos.'),
                ...
                if recordlength < lenfixed and self.ta_info['checkfixedrecordtoolong']:
                    raise botslib.InMessageError(
                        _('[S53] line %(line)s: Record "%(record)s" too long; is %(pos)s pos, defined is %(defpos)s pos.'),
                        line=lex_record[ID][LIN],
                        record=lex_record[ID][VALUE],
                        pos=lenfixed,
                        defpos=recordlength
...
class var(Inmessage):
    ...
    def _lex(self):
        ...
        if mode_inrecord and self.ta_info.get('allow_lastrecordnotclosedproperly', False):
            ...
            if leftover:
                raise botslib.InMessageError(
                    _('[A51]: Found non-valid data at end of edi file; probably a problem with separators or message structure: "%(leftover)s".'),
                    {'leftover': leftover}
                )
        ...
        for lex_field in lex_record:
            ...
            if not lex_field[SFIELD]:  # preceded by field-separator
                try:
                    ...
                    self.add2errorlist(
                        _('[F19] line %(line)s pos %(pos)s: Record "%(record)s" too many fields in record; unknown field "%(content)s".\n') %
                        {
                ...
                try:
                    ...
                    self.add2errorlist(
                        _('[F17] line %(line)s pos %(pos)s: Record "%(record)s" expect field but "%(content)s" is a subfield.\n') %
                        {
                            'content': lex_field[VALUE],
                            'line': lex_field[LIN],
                            'pos': lex_field[POS],
                    ...
                    self.add2errorlist(
                        _('[F18] line %(line)s pos %(pos)s: Record "%(record)s" too many subfields in composite; unknown subfield "%(content)s".\n') %
                        {
                            'content': lex_field[VALUE],
                            'line': lex_field[LIN],
                            'pos': lex_field[POS],
                ...
                if field_definition[MAXREPEAT] == 1:
                    if 'ISA' == self.mpathformat(record_definition[MPATH]) and field_definition[ID] == 'ISA11':  # exception for ISA
                        ...
                        self.add2errorlist(
                            _('[F40] line %(line)s pos %(pos)s: Record "%(record)s" expect not-repeating elemen, but "%(content)s" is repeating.\n') %
                            {
                                'content': lex_field[VALUE],
                                'line': lex_field[LIN],
                                'pos': lex_field[POS],
    ...
    def checkenvelope(self):
        ...
        for nodeunb in self.getloop({'BOTSID': 'UNB'}):
            ...
            if unbreference and unzreference and unbreference != unzreference:
                self.add2errorlist(
                    _('[E01]: UNB-reference is "%(unbreference)s"; should be equal to UNZ-reference "%(unzreference)s".\n') %
            ...
            try:
                if int(unzcount) != messagecount:
                    self.add2errorlist(
                        _('[E02]: Count of messages in UNZ is %(unzcount)s; should be equal to number of messages %(messagecount)s.\n') %
            ...
            for nodeunh in nodeunb.getloop({'BOTSID': 'UNB'}, {'BOTSID': 'UNH'}):
                ...
                if unhreference and untreference and unhreference != untreference:
                    self.add2errorlist(
                        _('[E04]: UNH-reference is "%(unhreference)s"; should be equal to UNT-reference "%(untreference)s".\n') %
                ...
                try:
                    if int(untcount) != segmentcount:
                        self.add2errorlist(
                            _('[E05]: Segmentcount in UNT is %(untcount)s; should be equal to number of segments %(segmentcount)s.\n') %
            ...
            for nodeung in nodeunb.getloop({'BOTSID': 'UNB'}, {'BOTSID': 'UNG'}):
                ...
                if ungreference and unereference and ungreference != unereference:
                    self.add2errorlist(
                        _('[E07]: UNG-reference is "%(ungreference)s"; should be equal to UNE-reference "%(unereference)s".\n') %
                ...
                try:
                    if int(unecount) != groupcount:
                        self.add2errorlist(
                            _('[E08]: Groupcount in UNE is %(unecount)s; should be equal to number of groups %(groupcount)s.\n') %
                ...
                for nodeunh in nodeung.getloop({'BOTSID': 'UNG'}, {'BOTSID': 'UNH'}):
                    ...
                    if unhreference and untreference and unhreference != untreference:
                        self.add2errorlist(
                            _('[E10]: UNH-reference is "%(unhreference)s"; should be equal to UNT-reference "%(untreference)s".\n') %
                            {'unhreference': unhreference, 'untreference': untreference}
                        )
                    untcount = nodeunh.get({'BOTSID': 'UNH'}, {'BOTSID': 'UNT', '0074': None})
                    segmentcount = nodeunh.getcount()
                    try:
                        if int(untcount) != segmentcount:
                            self.add2errorlist(
                                _('[E11]: Segmentcount in UNT is %(untcount)s; should be equal to number of segments %(segmentcount)s.\n') %
                                {'untcount': untcount, 'segmentcount': segmentcount}
                            )
                    except Exception:
                        self.add2errorlist(
        ...
        for char in rawinput:
            ...
            if count <= 3:
                ...
                if char != self.ta_info['field_sep']:
                    raise botslib.InMessageError(
                        _('[A63]: Non-valid ISA header; position %(pos)s of ISA is "%(foundchar)s", expect here element separator "%(field_sep)s".'),
    ...
    def checkenvelope(self):
        ...
        for nodeisa in self.getloop({'BOTSID': 'ISA'}):
            ...
            if isareference and ieareference and isareference != ieareference:
                self.add2errorlist(
                    _('[E13]: ISA-reference is "%(isareference)s"; should be equal to IEA-reference "%(ieareference)s".\n') %
            ...
            try:
                if int(ieacount) != groupcount:
                    self.add2errorlist(
                        _('[E14]: Count in IEA-IEA01 is %(ieacount)s; should be equal to number of groups %(groupcount)s.\n') %
            ...
            for nodegs in nodeisa.getloop({'BOTSID': 'ISA'}, {'BOTSID': 'GS'}):
                ...
                if gsreference and gereference and gsreference != gereference:
                    self.add2errorlist(
                        _('[E16]: GS-reference is "%(gsreference)s"; should be equal to GE-reference "%(gereference)s".\n') %
                ...
                try:
                    if int(gecount) != messagecount:
                        self.add2errorlist(
                            _('[E17]: Count in GE-GE01 is %(gecount)s; should be equal to number of transactions: %(messagecount)s.\n') %
                ...
                for nodest in nodegs.getloop({'BOTSID': 'GS'}, {'BOTSID': 'ST'}):
                    ...
                    if streference and sereference and streference != sereference:
                        self.add2errorlist(
                            _('[E19]: ST-reference is "%(streference)s"; should be equal to SE-reference "%(sereference)s".\n') %
                    ...
                    try:
                        if int(secount) != segmentcount:
                            self.add2errorlist(
                                _('[E20]: Count in SE-SE01 is %(secount)s; should be equal to number of segments %(segmentcount)s.\n') %
        ...
        for nodestx in self.getloop({'BOTSID': 'STX'}):
            ...
            try:
                if int(endcount) != messagecount:
                    self.add2errorlist(
                        _('[E22]: Count in END is %(endcount)s; should be equal to number of messages %(messagecount)s.\n') %
            ...
            for nodemhd in nodestx.getloop({'BOTSID': 'STX'}, {'BOTSID': 'MHD'}):
                ...
                try:
                    if int(mtrcount) != segmentcount:
                        self.add2errorlist(
                            _('[E24]: Count in MTR is %(mtrcount)s; should be equal to number of segments %(segmentcount)s.\n') %
