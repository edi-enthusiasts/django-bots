    ...
    try:
        ...
        ediobject.errorlist.append(str(
            botslib.InMessageError(
                _(u'[A59]: incoming file has not allowed characters at/after file-position %(pos)s: "%(content)s".'),
    ...
    def initfromfile(self):
        ...
        if leftover:
            raise botslib.InMessageError(
                _(u'[A50] line %(line)s pos %(pos)s: Found non-valid data at end of edi file; probably a problem with separators or message structure.'),
    ...
    def _formatfield(self, value, field_definition, structure_record, node_instance):
        ...
        if field_definition[BFORMAT] == 'A':
            if len(value) > field_definition[LENGTH]:
                self.add2errorlist(
                    _(u'[F05]%(linpos)s: Record "%(record)s" field "%(field)s" too big (max %(max)s): "%(content)s".\n') %
                    {
            ...
            if len(value) < field_definition[MINLENGTH]:
                self.add2errorlist(
                    _(u'[F06]%(linpos)s: Record "%(record)s" field "%(field)s" too small (min %(min)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if field_definition[BFORMAT] == 'D':
                try:
                    ...
                    self.add2errorlist(
                        _(u'[F07]%(linpos)s: Record "%(record)s" date field "%(field)s" not a valid date: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                try:
                    ...
                    self.add2errorlist(
                        _(u'[F08]%(linpos)s: Record "%(record)s" time field "%(field)s" not a valid time: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
            ...
            if length > field_definition[LENGTH]:
                self.add2errorlist(
                    _(u'[F10]%(linpos)s: Record "%(record)s" field "%(field)s" too big (max %(max)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if length < field_definition[MINLENGTH]:
                self.add2errorlist(
                    _(u'[F11]%(linpos)s: Record "%(record)s" field "%(field)s" too small (min %(min)s): "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if 'E' in value or 'e' in value:
                self.add2errorlist(
                    _(u'[F09]%(linpos)s: Record "%(record)s" field "%(field)s" contains exponent: "%(content)s".\n') %
                    {
                        'linpos': node_instance.linpos(),
                        'record': self.mpathformat(structure_record[MPATH]),
                        'field': field_definition[ID],
            ...
            if field_definition[BFORMAT] == 'R':
                ...
                try:  # convert to decimal in order to check validity
                    ...
                    self.add2errorlist(
                        _(u'[F16]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                if lendecimal != field_definition[DECIMALS]:
                    self.add2errorlist(
                        _(u'[F14]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has invalid nr of decimals: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                try:  # convert to decimal in order to check validity
                    ...
                    self.add2errorlist(
                        _(u'[F15]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                ...
                if '.' in value:
                    self.add2errorlist(
                        _(u'[F12]%(linpos)s: Record "%(record)s" field "%(field)s" has format "I" but contains decimal sign: "%(content)s".\n') %
                        {
                            'linpos': node_instance.linpos(),
                            'record': self.mpathformat(structure_record[MPATH]),
                            'field': field_definition[ID],
                    ...
                    try:  # convert to decimal in order to check validity
                        ...
                        self.add2errorlist(
                            _(u'[F13]%(linpos)s: Record "%(record)s" numeric field "%(field)s" has non-numerical content: "%(content)s".\n') %
                            {
                                'linpos': node_instance.linpos(),
                                'record': self.mpathformat(structure_record[MPATH]),
                                'field': field_definition[ID],
            ...
            if current_lex_record is None or structure_level[structure_index][ID] != current_lex_record[ID][VALUE]:
                if structure_level[structure_index][MIN] and not countnrofoccurences:  # is record is required in structure_level, and countnrofoccurences==0: error;
                    ...
                    try:
                        raise botslib.InMessageError(
                            self.messagetypetxt +
                            _(u'[S50]: Line:%(line)s pos:%(pos)s record:"%(record)s": message has an error in its structure; this record is not allowed here. Scanned in message definition until mandatory record: "%(looked)s".'),
                            {
                        ...
                        raise botslib.InMessageError(
                            self.messagetypetxt +
                            _(u'[S51]: Missing mandatory record "%(record)s".'),
            ...
            if SUBTRANSLATION in structure_level[structure_index]:
                ...
                if not messagetype:
                    raise botslib.TranslationNotFoundError(
                        _(u'Could not find SUBTRANSLATION "%(sub)s" in (sub)message.'),
                ...
                try:
                    ...
                    if raisenovalidmapping_error:
                        raise botslib.TranslationNotFoundError(
                            _(u'No (valid) grammar for editype "%(editype)s" messagetype "%(messagetype)s".'),
...
class fixed(Inmessage):
    ...
    def _lex(self):
        ...
        try:
            ...
            content = botslib.get_relevant_text_for_UnicodeError(msg)
            raise botslib.InMessageError(
                _(u'Characterset problem in file. At/after line %(line)s: "%(content)s"'),
        ...
        if self.ta_info['noBOTSID']:  # if read records contain no BOTSID: add it
            ...
            if recordlength != lenfixed:
                if recordlength > lenfixed and self.ta_info['checkfixedrecordtooshort']:
                    raise botslib.InMessageError(
                        _(u'[S52] line %(line)s: Record "%(record)s" too short; is %(pos)s pos, defined is %(defpos)s pos.'),
                ...
                if recordlength < lenfixed and self.ta_info['checkfixedrecordtoolong']:
                    raise botslib.InMessageError(
                        _(u'[S53] line %(line)s: Record "%(record)s" too long; is %(pos)s pos, defined is %(defpos)s pos.'),
                        line=lex_record[ID][LIN],
                        record=lex_record[ID][VALUE],
                        pos=lenfixed,
                        defpos=recordlength
...
class var(Inmessage):
    ...
    def _lex(self):
        ...
        if mode_inrecord and self.ta_info.get('allow_lastrecordnotclosedproperly', False):
            ...
            if leftover:
                raise botslib.InMessageError(
                    _(u'[A51]: Found non-valid data at end of edi file; probably a problem with separators or message structure: "%(leftover)s".'),
                    {'leftover': leftover}
                )
        ...
        for lex_field in lex_record:
            ...
            if not lex_field[SFIELD]:  # preceded by field-separator
                try:
                    ...
                    self.add2errorlist(
                        _(u'[F19] line %(line)s pos %(pos)s: Record "%(record)s" too many fields in record; unknown field "%(content)s".\n') %
                        {
                ...
                try:
                    ...
                    self.add2errorlist(
                        _(u'[F17] line %(line)s pos %(pos)s: Record "%(record)s" expect field but "%(content)s" is a subfield.\n') %
                        {
                            'content': lex_field[VALUE],
                            'line': lex_field[LIN],
                            'pos': lex_field[POS],
                    ...
                    self.add2errorlist(
                        _(u'[F18] line %(line)s pos %(pos)s: Record "%(record)s" too many subfields in composite; unknown subfield "%(content)s".\n') %
                        {
                            'content': lex_field[VALUE],
                            'line': lex_field[LIN],
                            'pos': lex_field[POS],
                ...
                if field_definition[MAXREPEAT] == 1:
                    if 'ISA' == self.mpathformat(record_definition[MPATH]) and field_definition[ID] == 'ISA11':  # exception for ISA
                        ...
                        self.add2errorlist(
                            _(u'[F40] line %(line)s pos %(pos)s: Record "%(record)s" expect not-repeating elemen, but "%(content)s" is repeating.\n') %
                            {
                                'content': lex_field[VALUE],
                                'line': lex_field[LIN],
                                'pos': lex_field[POS],
...
class excel(csv):
    ...
    def initfromfile(self):
        ...
        if leftover:
            raise botslib.InMessageError(
                _(u'[A52]: Found non-valid data at end of excel file: "%(leftover)s".'),
                {'leftover': leftover}
            )
        del self.lex_records
        self.checkmessage(self.root, self.defmessage)
    ...
    def read_xls(self, infilename):
        ...
        formatter = lambda(t, v): self.format_excelval(book, t, v, False)  # @IgnorePep8
        xlsdata = []
...
class edifact(var):
    ...
    def _sniff(self):
        ...
        try:
            ...
            raise botslib.InMessageError(
                _(u'[A58]: Edifact file has unknown characterset "%(charset)s".'),
                {'charset': found_charset}
            )
        # except UnicodeDecodeError as msg:
            # raise botslib.InMessageError(
            #     _(u'[A59]: Edifact file has not allowed characters at/after file-position %(content)s.'),
            #     {'content': msg[2]}
            # )
    ...
    def checkenvelope(self):
        ...
        for nodeunb in self.getloop({'BOTSID': 'UNB'}):
            ...
            if unbreference and unzreference and unbreference != unzreference:
                self.add2errorlist(
                    _(u'[E01]: UNB-reference is "%(unbreference)s"; should be equal to UNZ-reference "%(unzreference)s".\n') %
            ...
            try:
                if int(unzcount) != messagecount:
                    self.add2errorlist(
                        _(u'[E02]: Count of messages in UNZ is %(unzcount)s; should be equal to number of messages %(messagecount)s.\n') %
                        {'unzcount': unzcount, 'messagecount': messagecount}
                    )
            except Exception:
                self.add2errorlist(
                    _(u'[E03]: Count of messages in UNZ is invalid: "%(count)s".\n') %
                    {'count': unzcount}
                )
            for nodeunh in nodeunb.getloop({'BOTSID': 'UNB'}, {'BOTSID': 'UNH'}):
                ...
                if unhreference and untreference and unhreference != untreference:
                    self.add2errorlist(
                        _(u'[E04]: UNH-reference is "%(unhreference)s"; should be equal to UNT-reference "%(untreference)s".\n') %
                ...
                try:
                    if int(untcount) != segmentcount:
                        self.add2errorlist(
                            _(u'[E05]: Segmentcount in UNT is %(untcount)s; should be equal to number of segments %(segmentcount)s.\n') %
                            {'untcount': untcount, 'segmentcount': segmentcount}
                        )
                except Exception:
                    self.add2errorlist(
                        _(u'[E06]: Count of segments in UNT is invalid: "%(count)s".\n') %
                        {'count': untcount}
                    )
            for nodeung in nodeunb.getloop({'BOTSID': 'UNB'}, {'BOTSID': 'UNG'}):
                ...
                if ungreference and unereference and ungreference != unereference:
                    self.add2errorlist(
                        _(u'[E07]: UNG-reference is "%(ungreference)s"; should be equal to UNE-reference "%(unereference)s".\n') %
                ...
                try:
                    if int(unecount) != groupcount:
                        self.add2errorlist(
                            _(u'[E08]: Groupcount in UNE is %(unecount)s; should be equal to number of groups %(groupcount)s.\n') %
                            {'unecount': unecount, 'groupcount': groupcount}
                        )
                except Exception:
                    self.add2errorlist(
                        _(u'[E09]: Groupcount in UNE is invalid: "%(count)s".\n') %
                        {'count': unecount}
                    )
                for nodeunh in nodeung.getloop({'BOTSID': 'UNG'}, {'BOTSID': 'UNH'}):
                    ...
                    if unhreference and untreference and unhreference != untreference:
                        self.add2errorlist(
                            _(u'[E10]: UNH-reference is "%(unhreference)s"; should be equal to UNT-reference "%(untreference)s".\n') %
                            {'unhreference': unhreference, 'untreference': untreference}
                        )
                    untcount = nodeunh.get({'BOTSID': 'UNH'}, {'BOTSID': 'UNT', '0074': None})
                    segmentcount = nodeunh.getcount()
                    try:
                        if int(untcount) != segmentcount:
                            self.add2errorlist(
                                _(u'[E11]: Segmentcount in UNT is %(untcount)s; should be equal to number of segments %(segmentcount)s.\n') %
                                {'untcount': untcount, 'segmentcount': segmentcount}
                            )
                    except Exception:
                        self.add2errorlist(
                            _(u'[E12]: Count of segments in UNT is invalid: "%(count)s".\n') %
                            {'count': untcount}
                        )
    ...
    def handleconfirm(self, ta_fromfile, error):
        ...
        for nodeunb in self.getloop({'BOTSID': 'UNB'}):
            ...
            botsglobal.logger.debug(
                u'Send edifact confirmation (CONTRL) route "%(route)s" fromchannel "%(fromchannel)s" frompartner "%(frompartner)s" topartner "%(topartner)s".',
...
class x12(var):
    ...
    def _sniff(self):
        ...
        for char in rawinput:
            ...
            if count <= 3:
                ...
                if recordID != 'ISA':
                    raise botslib.InMessageError(
                        _(u'[A60]: Expect "ISA", found "%(content)s". Probably no x12?'),
                        {'content': self.rawinput[:7]}
                    )  # not with mailbag
            elif count in [7, 18, 21, 32, 35, 51, 54, 70]:  # extra checks for fixed ISA.
                if char != self.ta_info['field_sep']:
                    raise botslib.InMessageError(
                        _(u'[A63]: Non-valid ISA header; position %(pos)s of ISA is "%(foundchar)s", expect here element separator "%(field_sep)s".'),
    ...
    def checkenvelope(self):
        ...
        for nodeisa in self.getloop({'BOTSID': 'ISA'}):
            ...
            if isareference and ieareference and isareference != ieareference:
                self.add2errorlist(
                    _(u'[E13]: ISA-reference is "%(isareference)s"; should be equal to IEA-reference "%(ieareference)s".\n') %
            ...
            try:
                if int(ieacount) != groupcount:
                    self.add2errorlist(
                        _(u'[E14]: Count in IEA-IEA01 is %(ieacount)s; should be equal to number of groups %(groupcount)s.\n') %
                        {'ieacount': ieacount, 'groupcount': groupcount}
                    )
            except Exception:
                self.add2errorlist(
                    _(u'[E15]: Count of messages in IEA is invalid: "%(count)s".\n') %
                    {'count': ieacount}
                )
            for nodegs in nodeisa.getloop({'BOTSID': 'ISA'}, {'BOTSID': 'GS'}):
                ...
                if gsreference and gereference and gsreference != gereference:
                    self.add2errorlist(
                        _(u'[E16]: GS-reference is "%(gsreference)s"; should be equal to GE-reference "%(gereference)s".\n') %
                ...
                try:
                    if int(gecount) != messagecount:
                        self.add2errorlist(
                            _(u'[E17]: Count in GE-GE01 is %(gecount)s; should be equal to number of transactions: %(messagecount)s.\n') %
                            {'gecount': gecount, 'messagecount': messagecount}
                        )
                except Exception:
                    self.add2errorlist(
                        _(u'[E18]: Count of messages in GE is invalid: "%(count)s".\n') %
                        {'count': gecount}
                    )
                for nodest in nodegs.getloop({'BOTSID': 'GS'}, {'BOTSID': 'ST'}):
                    ...
                    if streference and sereference and streference != sereference:
                        self.add2errorlist(
                            _(u'[E19]: ST-reference is "%(streference)s"; should be equal to SE-reference "%(sereference)s".\n') %
                    ...
                    try:
                        if int(secount) != segmentcount:
                            self.add2errorlist(
                                _(u'[E20]: Count in SE-SE01 is %(secount)s; should be equal to number of segments %(segmentcount)s.\n') %
                                {'secount': secount, 'segmentcount': segmentcount}
                            )
                    except Exception:
                        self.add2errorlist(
                            _(u'[E21]: Count of segments in SE is invalid: "%(count)s".\n') %
                            {'count': secount}
                        )
    ...
    def handleconfirm(self, ta_fromfile, error):
        ...
        for nodegs in self.getloop({'BOTSID': 'ISA'}, {'BOTSID': 'GS'}):
            ...
            botsglobal.logger.debug(
                u'Send x12 confirmation (997) route "%(route)s" fromchannel "%(fromchannel)s" frompartner "%(frompartner)s" topartner "%(topartner)s".',
                {
        ...
        for nodestx in self.getloop({'BOTSID': 'STX'}):
            ...
            try:
                if int(endcount) != messagecount:
                    self.add2errorlist(
                        _(u'[E22]: Count in END is %(endcount)s; should be equal to number of messages %(messagecount)s.\n') %
            ...
            for nodemhd in nodestx.getloop({'BOTSID': 'STX'}, {'BOTSID': 'MHD'}):
                ...
                try:
                    if int(mtrcount) != segmentcount:
                        self.add2errorlist(
                            _(u'[E24]: Count in MTR is %(mtrcount)s; should be equal to number of segments %(segmentcount)s.\n') %
        ...
        for xmlchildnode in xmlnode:  # for every node in mpathtree
            ...
            if not entitytype:  # is a field, or unknown that looks like a field
                ...
                if self.ta_info['checkunknownentities']:
                    self.add2errorlist(
                        _(u'[S02]%(linpos)s: Unknown xml-tag "%(recordunkown)s" (within "%(record)s") in message.\n') %
    ...
    def _dojsonobject(self, jsonobject, name):
        ...
        for key, value in jsonobject.iteritems():
            if value is None:
                ...
                if self.ta_info['checkunknownentities']:
                    raise botslib.InMessageError(
                        _(u'[J55]: Key "%(key)s" value "%(value)s": is not string, list or dict.'),
                        {'key': key, 'value': value}
                    )
                thisnode.record[key] = str(value)
