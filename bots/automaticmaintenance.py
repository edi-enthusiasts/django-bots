...
from .botsconfig import (
    ...
    EXTERNIN, EXTERNOUT,
    FILEIN, OPEN,
    PARSED, PROCESS, SPLITUP, TRANSLATED
)
...
def make_run_report(rootidtaofrun, resultsofrun, command, totalfilesize):
    ...
    botslib.changeq(
        '''INSERT INTO report (
            ...
            VALUES (
                %(rootidtaofrun)s, %(lastopen)s, %(lasterror)s, %(lastok)s, %(lastdone)s, %(send)s, %(processerrors)s,
...
def email_error_report(rootidtaofrun):
    for results in botslib.query(
        ...
        raise botslib.PanicError(_t('In generate report: could not find report?'))

    subject = _t('[Bots Error Report] %(time)s') % {'time': str(results['ts'])[:16]}
    reporttext = _t('Bots Report; type: %(type)s, time: %(time)s\n') % {'type': results['type'], 'time': str(results['ts'])[:19]}
    reporttext += _t('    %d files received/processed in run.\n') % results['lastreceived']
    if results['lastdone']:
        reporttext += _t('    %d files without errors,\n') % results['lastdone']
    if results['lasterror']:
        subject += _t('; %d file errors') % results['lasterror']
        reporttext += _t('    %d files with errors,\n') % results['lasterror']
    if results['lastok']:
        subject += _t('; %d files stuck') % results['lastok']
        reporttext += _t('    %d files got stuck,\n') % results['lastok']
    if results['lastopen']:
        subject += _t('; %d system errors') % results['lastopen']
        reporttext += _t('    %d system errors,\n') % results['lastopen']
    if results['processerrors']:
        subject += _t('; %d process errors') % results['processerrors']
        reporttext += _t('    %d errors in processes.\n') % results['processerrors']
    reporttext += _t('    %d files send in run.\n') % results['send']
    ...
    if results['lasterror'] or results['lastopen'] or results['lastok'] or (results['processerrors'] and botsglobal.ini.getboolean('settings', 'sendreportifprocesserror', fallback=True)):
        ...
        if results['processerrors']:
            for row in botslib.query(
                '''SELECT idroute, fromchannel, tochannel, errortext FROM ta
                   WHERE idta >= %(rootidtaofrun)s
                   AND status = %(status)s
                   AND statust = %(statust)s ''',
        ...
        if results['lasterror'] or results['lastopen'] or results['lastok']:
            for row in botslib.query(
                '''SELECT idroute, frompartner, fromchannel,
                          topartner, tochannel, errortext, infilename
                   FROM filereport
                   WHERE idta > %(rootidtaofrun)s
                   AND statust != %(statust)s ''',
...
class Trace(object):
    ...
    def __init__(self, row, rootidtaofrun):
        ...
        try:
            ...
            botsglobal.logger.exception(_t('Error in automatic maintenance: "%(msg)s".'), {'msg': msg})
            self.statust = OPEN
    ...
    def _getstatusfortreeoftransactions(self, tacurrent):
        ...
        if tacurrent['statust'] == DONE:
            ...
            if tacurrent['talijst']:
                raise botslib.TraceError(_t('Statust OK (stuck) but has child(ren) (idta: %(idta)s).'), tacurrent)
            else:
                return OK
        elif tacurrent['statust'] == ERROR:  # should be no children.
            if tacurrent['talijst']:
                raise botslib.TraceError(_t('Statust ERROR but has child(ren) (idta: %(idta)s).'), tacurrent)
            else:
                return ERROR
        else:  # tacurrent.statust==OPEN: something is very wrong. Raise exception.
            raise botslib.TraceError(_t('Severe error: found statust OPEN for idta: %(idta)s.'), tacurrent)
    ...
    def make_file_report(self):
        ...
        botslib.changeq(
            '''INSERT INTO filereport (
                    ...
                    outcontenttype, nrmessages, outfilename, errortext, divtext, outidta,
                    rsrv1, filesize
                )
                VALUES (
                    %(idta)s ,%(statust)s, %(reportidta)s, %(retransmit)s, %(idroute)s, %(fromchannel)s, %(ts)s,
                    %(infilename)s, %(tochannel)s, %(frompartner)s, %(topartner)s, %(frommail)s, %(tomail)s,
                    %(ineditype)s, %(inmessagetype)s, %(outeditype)s, %(outmessagetype)s, %(incontenttype)s,
                    %(outcontenttype)s, %(nrmessages)s, %(outfilename)s, %(errortext)s, %(divtext)s, %(outidta)s,
                    %(rsrv1)s, %(filesize)s
                )''',
            tmp_dict
        )
