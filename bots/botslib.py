...
try:
    ...
    from . import bots_importlib as importlib  # @Reimport
...
from . import botsglobal   # globals
...
class _Transaction(object):
    ...
    filterlist = (
        'statust', 'status', 'divtext', 'parent', 'child', 'script', 'frompartner', 'topartner', 'fromchannel', 'tochannel', 'editype', 'messagetype', 'merge',
    ...
    def update(self, **ta_info):
        ...
        changeq(
            u'''UPDATE ta SET '''+setstring+''' WHERE idta=%(selfid)s''',
            ta_info
        )
    ...
    def delete(self):
        ...
        changeq(
            u'''DELETE FROM ta WHERE idta=%(idta)s''',
            {'idta': self.idta}
        )
    ...
    def copyta(self, status, **ta_info):
        ...
        newidta = insertta(
            u'''INSERT INTO ta (
                    script, status, parent, frompartner, topartner, fromchannel, tochannel,
                    editype, messagetype, alt, merge, testindicator, reference, frommail,
                    tomail, charset, contenttype, filename, idroute, nrmessages, botskey,
                    envelope, rsrv3
                )
                SELECT %(script)s, %(newstatus)s, idta, frompartner, topartner, fromchannel, tochannel,
                       editype, messagetype, alt, merge, testindicator, reference, frommail,
                       tomail, charset, contenttype, filename, idroute, nrmessages, botskey,
                       envelope, rsrv3
                FROM ta
...
def prepare_confirmrules():
    ...
    for confirmdict in query(
        u'''SELECT confirmtype, ruletype, idroute,
...
def lookup_translation(frommessagetype, fromeditype, alt, frompartner, topartner):
    ...
    '''
    for row2 in query(
        u'''SELECT tscript, tomessagetype, toeditype FROM translate
            WHERE frommessagetype = %(frommessagetype)s
            AND fromeditype = %(fromeditype)s
            AND active = %(booll)s
            AND (alt='' OR alt=%(alt)s)
            AND (frompartner_id IS NULL OR frompartner_id=%(frompartner)s OR frompartner_id in (SELECT to_partner_id FROM partnergroup WHERE from_partner_id=%(frompartner)s))
            AND (topartner_id IS NULL OR topartner_id=%(topartner)s OR topartner_id in (SELECT to_partner_id FROM partnergroup WHERE from_partner_id=%(topartner)s))
            ORDER BY
                alt DESC,
                CASE WHEN frompartner_id IS NULL THEN 1 ELSE 0 END,
                frompartner_id,
                CASE WHEN topartner_id IS NULL THEN 1 ELSE 0 END,
                topartner_id''',
        {
...
class Uri(object):
    ...
    def __str__(self):
        ...
        port     = u':' + str(self._uri['port']) if self._uri['port'] else u''
