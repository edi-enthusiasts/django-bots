...
def start():
    ...
    if botsglobal.ini.getboolean('acceptance', 'runacceptancetest', False):
        ...
        try:
            ...
            botsglobal.logger.info(_t('In acceptance test there is no script file "bots_acceptancetest.py" to check the results of the acceptance test.'))
...
def get_control_information():
    ...
    #     editype='edifact',
    #     messagetype='edifact',
    #     tscript='orders_edifact2xml',
    #     toeditype='xml',
    #     tomessagetype='orders'
    # )
    return run
...
def translate(run):
    for messagedict in run.incoming:
        try:
            ...
            for inn_splitup in edifile.nextmessage():  # s plitup messages in parsed edifile
                try:
                    ...
                    while 1:  # continue as long as there are (alt-)translations
                        ...
                        if doalttranslation is None:
                            ...
                            if 'type' not in doalttranslation or 'alt' not in doalttranslation:
                                raise botslib.BotsError(_t("Mappingscript returned '%(alt)s'. This dict should not have 'type' and 'alt'."), {'alt': doalttranslation})
                            ...
                            if doalttranslation['type'] == 'out_as_inn':
                                ...
                                transform.handle_out_message(out_translated, ta_translated)
                                ...
                                if 'frompartner' not in inn_splitup.ta_info:
                                    inn_splitup.ta_info['frompartner'] = ''
                                if 'topartner' not in inn_splitup.ta_info:
                                ...
                                raise botslib.BotsError(_t('Mappingscript returned dict with an unknown "type": "%(doalttranslation)s".'), {'doalttranslation': doalttranslation})
                        ...
                        if number_of_loops_with_same_alt > 10:
                            raise botslib.BotsError(_t('Mappingscript returns same alt value over and over again (infinite loop?). Alt: "%(doalttranslation)s".'), {'doalttranslation': doalttranslation})
...
def filter(lijst, names):  # @ReservedAssignment
...
def report(run):
    ...
    xml_string = dict2xml({
        'root': {
            'nr_errors': run.errorinrun,
            'incoming': list(filter(run.incoming, in_filter)),
            'outgoing': list(filter(run.outgoing, out_filter))
        }
    })
    print(xml_string)


def cleanup(run):  # @UnusedVariable
